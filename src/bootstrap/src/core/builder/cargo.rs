...
impl Builder<'_> {
    ...
    fn cargo(
        ...
        let trim_paths = |cargo: &mut BootstrapCommand, ws_remap: &str| {
            ...
            // The profile keys must be `--config` arguments, not
            // `CARGO_PROFILE_*` environment variables: build scripts may
            // invoke a nested cargo (e.g. rust-analyzer's proc-macro-test),
            // which inherits the environment but not the `-Ztrim-paths`
            // gate, and would refuse the unstable profile key.
            cargo.arg("--config").arg("profile.release.trim-paths=\"object\"");
            cargo.arg("--config").arg("profile.dev.trim-paths=\"object\"");
